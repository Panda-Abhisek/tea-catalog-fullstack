      ...
      <div className="max-w-7xl mx-auto p-6">
        <div className="animate-pulse">
          ...
          <div className="grid grid-cols-4 gap-4">
            {[1, 2, 3, 4].map((item) => (
              <div key={item} className="h-28 bg-gray-200 rounded-xl" />
            ))}
          </div>
        </div>
      </div>
    ...
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-8">
        <div className="bg-amber-100/70 backdrop-blur-sm p-5 rounded-xl shadow">
          <h3 className="text-sm text-gray-500">Total Teas</h3>
          <p className="text-3xl font-bold">{stats.total_teas}</p>
        </div>
        ...
        <div className="bg-amber-100/70 backdrop-blur-sm p-5 rounded-xl shadow">
          ...
          <p className="text-3xl font-bold">₹{stats.inventory_value}</p>
        </div>

        <div className="bg-amber-100/70 backdrop-blur-sm p-5 rounded-xl shadow">
          <h3 className="text-sm text-gray-500">Average Tea Price</h3>
          <p className="text-3xl font-bold">₹{Math.round(stats.avg_price)}</p>
        </div>
      </div>
        ...
        {stats.low_stock_teas.length === 0 ? (
          ...
          <div className="space-y-3">
            {stats.low_stock_teas.map((tea) => (
              <div key={tea.id} className="flex justify-between border-b pb-2">
                <span>{tea.name}</span>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="bg-amber-100/70 backdrop-blur-sm rounded-xl shadow p-5">
          <h2 className="text-xl font-semibold mb-4">Latest Teas</h2>

          <div className="space-y-3">
            {stats.latest_teas.map((tea) => (
              <div key={tea.id} className="flex justify-between">
                <span>{tea.name}</span>
      ...
      <div className="bg-amber-100/70 backdrop-blur-sm rounded-xl shadow p-5 mt-8">
        <h2 className="text-xl font-semibold mb-4">Premium Teas</h2>
        ...
        <div className="space-y-3">
          {stats.top_expensive_teas.map((tea) => (
            <div key={tea.id} className="flex justify-between">
              ...
              <span className="font-semibold">₹{tea.price}</span>
            </div>
          ))}
        </div>
      </div>
