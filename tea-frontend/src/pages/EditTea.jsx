  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response?.status === 400 && err.response.data) {
        ...
        setErrors({ non_field_errors: ['Failed to update tea. Please try again.'] });
      }
    } finally {
  ...
  if (fetchError) {
    return (
      <div className="max-w-2xl mx-auto text-center py-12">
        ...
        <Link to="/admin/teas" className="btn-primary bg-emerald-600 hover:bg-emerald-700 focus:ring-emerald-500">
          Return to Inventory
        </Link>
      </div>
    );
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-amber-100">Update the details for this inventory item.</p>
        </div>
